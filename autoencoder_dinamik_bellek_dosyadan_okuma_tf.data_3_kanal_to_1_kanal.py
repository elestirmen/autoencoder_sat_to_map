...
all_image_paths = ["C:\\d_surucusu\\satmap\\output_full\\" + fname for fname in os.listdir("C:\\d_surucusu\\satmap\\output_full\\")]
#all_image_paths = ["C:\\d_surucusu\\satmap\\sonuclar_19\\" + fname for fname in os.listdir("C:\\d_surucusu\\satmap\\sonuclar_19\\")]

#all_image_paths = ["E:\\tez veri\\out_isvicre_ham\\" + fname for fname in os.listdir("E:\\tez veri\\out_isvicre_ham\\")]
...
def load_and_preprocess(image_path):
    ...
    #     return (tf.zeros([544, 544, 3], dtype=tf.float32), tf.zeros([544, 544, 3], dtype=tf.float32))
...
def create_deneysel_model(input_shape, activation_func="sigmoid", filtre_adet=48, kernel_size=(3,3), strds=(2, 2)):
    
    input_layer = Input(shape=input_shape)
    
    x = Conv2D(filtre_adet*16, kernel_size=(5,5), strides=(2, 2), activation='relu', padding='same', kernel_initializer='he_normal')(input_layer)   
    # x = Dropout(0.3)(x)
    x = Conv2D(filtre_adet*8, kernel_size=kernel_size, strides=(1, 1), activation='relu', padding='same', kernel_initializer='he_normal')(x)
    # x = Dropout(0.3)(x)
    

    
    x = Conv2DTranspose(filtre_adet*4, kernel_size=kernel_size, strides=(1, 1), activation='relu', padding='same', kernel_initializer='he_normal')(x)
    # x = Dropout(0.3)(x)
    x = Conv2DTranspose(filtre_adet*2, kernel_size=(5,5), strides=(2, 2), activation='relu', padding='same', kernel_initializer='he_normal')(x)
    # x = Dropout(0.3)(x)
...
def create_autoencoder_model_classic(input_shape, activation_func="sigmoid", filtre_adet=48, kernel_size=(3,3), strds=(2, 2)):
    
    input_layer = Input(shape=input_shape)
    
    x = Conv2D(filtre_adet*16, kernel_size=kernel_size, strides=(1, 1), activation='elu', padding='same', kernel_initializer='he_normal')(input_layer)   
    x = Dropout(0.3)(x)
    x = Conv2D(filtre_adet*8, kernel_size=kernel_size, strides=(1, 1), activation='elu', padding='same', kernel_initializer='he_normal')(x)
    x = Dropout(0.3)(x)
    

    
    x = Conv2DTranspose(filtre_adet*8, kernel_size=kernel_size, strides=(1, 1), activation='elu', padding='same', kernel_initializer='he_normal')(x)
    x = Dropout(0.3)(x)
    x = Conv2DTranspose(filtre_adet*16, kernel_size=kernel_size, strides=(1, 1), activation='elu', padding='same', kernel_initializer='he_normal')(x)
    x = Dropout(0.3)(x)
...
def create_autoencoder_model(input_shape, activation_func='sigmoid', filter_count=8, kernel_size=(5, 5), strides=(2, 2)):
    input_layer = Input(shape=input_shape)
    ...
    output_layer = Conv2D(1, kernel_size=kernel_size, strides=(
        1, 1), activation=activation_func, padding='same')(x)
...
def create_advanced_autoencoder(input_shape, activation_func='relu', filter_count=32, kernel_size=(3, 3), strides=(1, 1)):
    ...
    # Encoder
    x = Conv2D(filter_count//2, kernel_size=kernel_size, strides=strides, activation=activation_func, padding='same')(input_layer)
    x = MaxPooling2D((2, 2), padding='same')(x)
    x = Dropout(0.3)(x)
    
    x = Conv2D(filter_count , kernel_size=kernel_size, strides=strides, activation=activation_func, padding='same')(x)
    x = MaxPooling2D((2, 2), padding='same')(x)
    ...
    x = Conv2DTranspose(filter_count//2, kernel_size=kernel_size, strides=(1,1), activation=activation_func, padding='same')(x)
...
def create_gpt_autoencoder(input_shape, activation_func='relu', filter_count=32, kernel_size=(3, 3), strides=(1, 1)):  #favori
    ...
    x = Conv2D(filter_count/2, kernel_size, activation='elu', padding='same',activity_regularizer=regularizers.l1(1e-5))(input_layer)
    x = MaxPooling2D((2, 2), padding='same')(x)
    x = Dropout(0.4)(x)
    
    x = Conv2D(filter_count, kernel_size, activation='elu', padding='same',activity_regularizer=regularizers.l1(1e-5))(x)
    x = MaxPooling2D((2, 2), padding='same')(x)
    x = Dropout(0.4)(x)
...
model = create_advanced_autoencoder(input_shape, activation_func, filtre_adet, kernel_size, strds)


model = load_model("son_model.h5")
#model = load_model("GPU_model_f48_k5_epoch_00450_sigmoid_(2, 2)_.h5")
...
train_dataset = train_dataset.map(load_and_preprocess, num_parallel_calls=8).batch(batch_size).prefetch(buffer_size=16)  #(buffer_size=tf.data.experimental.AUTOTUNE)

val_dataset = tf.data.Dataset.from_tensor_slices(val_image_paths)
val_dataset = val_dataset.map(load_and_preprocess, num_parallel_calls=8).batch(batch_size).prefetch(buffer_size=16)
