...
all_image_paths = ["C:\\d_surucusu\\satnap\\output_ps_renkli\\" + fname for fname in os.listdir("C:\\d_surucusu\\satnap\\output_ps_renkli\\")]
...
split_at = int(len(all_image_paths) * 0.8)  # %80 eğitim, %20 doğrulama
...
def load_and_preprocess(image_path):
    # try:
        ...
        input_img = tf.slice(img, [0, 0, 0], [height, width, 3])
        label_img = tf.slice(img, [0, width, 0], [height, width, 3])
...
def create_autoencoder_model(input_shape, activation_func='sigmoid', filter_count=32, kernel_size=(5, 5), strides=(1, 1)):
    ...
    output_layer = Conv2D(3, kernel_size=kernel_size, strides=(
...
def create_gpt_autoencoder(input_shape, activation_func='relu', filter_count=32, kernel_size=(3, 3), strides=(1, 1)):  #favori
    ...
    output_layer = Conv2DTranspose(3, kernel_size, activation='sigmoid', padding='same')(x)
