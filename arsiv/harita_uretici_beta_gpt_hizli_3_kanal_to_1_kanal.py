...
def load_image(filename, size=(544, 544)):
    pixels = load_img(filename, target_size=size, color_mode="rgb")
...
def process_image(model, yol2, olusturulacak_dosya, filename):
    ...
    # Hata oluştuğunda hata kodunu alın ve işleyin
        print("Hata Kodu:", type(hata).__name__)
        print("Hata Mesajı:", str(hata))
        return

    filename_parcalar = olusturulacak_dosya + '/goruntu_{}.jpg'.format(filename[:-4])
    
    goruntu = gen_image[0].reshape(544, 544, 1)
    
    goruntu = 255 * goruntu
    # OpenCV'ye uygun hale getir
    #goruntu = cv2.cvtColor(goruntu, cv2.COLOR_BGR2RGB)

    # Piksel değerlerini 0 ile 255 arasında ayarla
    

    # OpenCV ile görüntüyü kaydet
    if cv2.imwrite(filename_parcalar, goruntu) == False:
